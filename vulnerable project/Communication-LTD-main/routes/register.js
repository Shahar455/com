  ...
  db.userDbConfig.query(
    ...
    function (error, results, fields) {
      if (error) {
        ...
        req.flash(
          "error",
          "An error occurred while checking if the email is already registered. Please try again later."
        );
        res.render("register.ejs", { messages: req.flash("error") });
      } else {
        const hash = bf.encrypt(password);
        ...
        const insertQuery =
          "INSERT INTO users (email, password, fullname, password_history) VALUES (?, ?, ?, ?)";
        db.userDbConfig.query(
          insertQuery,
